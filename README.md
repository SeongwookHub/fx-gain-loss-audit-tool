...
![Tests](https://img.shields.io/badge/pytest-47%20tests-yellow)
...
실제 회사 파일로 확인하려면 아래처럼 CLI 인자로 파일 경로와 결산 조건을 지정하세요
...
무작위로 뽑아 `OCR재확인표본`이라고 표시합니다. 같은 데이터를 넣으면 항상 같은 표본이
뽑힙니다(재현 가능). 이 표본을 감사인이 다시 확인했을 때 OCR이 틀린 게 나오면, 표본을
늘릴지 전체를 다시 볼지는 감사인이 직접 판단해야 합니다 — 이 부분은 자동으로 처리되지
않습니다.
...
샘플이 들어 있습니다. 정상 케이스와 아래처럼 일부러 넣은 오류 케이스가 섞여 있습니다.

> **참고**: 환율 자체는 실제 EXIM API 조회값이지만, 거래(모집단)와 증빙 이미지
> (`evidence/`)는 전부 학습·데모 목적으로 Claude를 통해 만든 가상 데이터입니다. 회사의 실제
> 회계시스템과 연동해서 모집단이 빠짐없이 다 반영됐는지 확인(완전성 대사)하는 절차나, 실제
> 증빙(계약서·은행 고시서 등)을 구해서 검증하는 절차는 들어 있지 않습니다 — 실무에 적용할
> 때는 별도로 필요합니다(아래 Roadmap 참고).
...
`tests/test_fx_verification_pipeline.py`에 pytest 기반 테스트 47개가 있습니다. 환율 조회
...
- **End-to-end 정답 대조 테스트**: `sample_data/`의 8개 샘플 거래를 `검증포인트_참고용.xlsx`
  정답지와 대조 — 정상 케이스 3건은 false positive 없이, 오류 케이스 5건은 전부 걸리는지 확인
...
- ☑ 감사조서(Word) 자동생성
- ☑ pytest 테스트 스위트 (47개, monkeypatch 기반)
...
├── tests/
│   ├── test_fx_verification_pipeline.py  # pytest 스위트 (47개)
│   ├── conftest.py
